...
function checkSmaDeviation(owt) {
  var averageOverLastReadingsCountN = 3;
  var vibrationReadings = owt.vibrationReadings;
  if (!vibrationReadings)
    return true;
  if (vibrationReadings.length < averageOverLastReadingsCountN)
    return true;

  var lastNReadingsTotal = 0;
  var i;
  for (i = vibrationReadings.length-1; i > vibrationReadings.length - averageOverLastReadingsCountN - 1; i--) {
    lastNReadingsTotal += vibrationReadings[i].valueHz;
    console.log("checkSmaDeviation : iterating vibrations readings - " + vibrationReadings[i].valueHz + " lastNReadingsTotal = " + lastNReadingsTotal);
  }

  var lastNReadingsMean = parseFloat(lastNReadingsTotal) / averageOverLastReadingsCountN;
  var allowableDeviation = owt.meanOfLastNReadings * 0.1; // allow 10% deviation, else there's a structural defect
  console.log("checkSmaDeviation : allowableDeviation = " + allowableDeviation);

  if (owt.meanOfLastNReadings === -1) { // is the first three readings for the OWT
    owt.meanOfLastNReadings = lastNReadingsMean;
    console.log("checkSmaDeviation : returning true - owt.meanOfLastNReadings = lastNReadingsMean = " + lastNReadingsMean);
    return true;
    ...
    owt.meanOfLastNReadings = lastNReadingsMean;
    console.log("checkSmaDeviation : returning false - owt.meanOfLastNReadings = lastNReadingsMean = " + lastNReadingsMean);
    return false;
  } else {
...
function setupDemo(setupDemo) {
  ...
  return getAssetRegistry(NS + '.Owt')
    ...
    .then(function (vibrationReadingRegistry) {
      vibrationReadings.forEach(function (vibrationReading, index) {

        vibrationReading.sensorPositionCode = 1;
        vibrationReading.valueHz = Math.random() * 100;
        vibrationReading.taken = randomDate(new Date(2016, 0, 1), new Date());

        if (Math.random() >= 0.5) {
          vibrationReading.isAggregated = true;
          vibrationReading.aggregationType = 'MEAN';
          vibrationReading.takenTo = randomDate(vibrationReading.taken, new Date());
        } else {
          vibrationReading.isAggregated = false;
        }
      });
      return vibrationReadingRegistry.addAll(vibrationReadings);
    })*/;
}
